void createLocalA(Matrix &A,mesh m){
    float taf = m.getParameter(TAF);
    A.at(0).at(0) += -taf/8;  A.at(0).at(1) += taf/8;
    A.at(1).at(0) += -taf/8;  A.at(1).at(1) += taf/8;
}

void createLocalB(Matrix &B,mesh m){
    float l = m.getParameter(ELEMENT_LENGTH);
    float k = m.getParameter(KAPA);
    B.at(0).at(0) += k/l;      B.at(0).at(1) += -k/l;
    B.at(1).at(0) += -k/l;     B.at(1).at(1) += k/l;
}

void createLocalC(Matrix &C,mesh m){
    float lamda = m.getParameter(LAMDA);
    C.at(0).at(0) += -(lamda)/3;    C.at(0).at(1) += (lamda)/3;
    C.at(1).at(0) += -(lamda)/3;    C.at(1).at(1) += (lamda)/3;
}

void createLocalD(Matrix &D,mesh m){
    float alfa = m.getParameter(ALFA);
    D.at(0).at(0) += -(alfa*3)/2;  D.at(0).at(1) += (alfa*3)/2;
    D.at(1).at(0) += -(alfa*3)/2;  D.at(1).at(1) += (alfa*3)/2;
}

void createLocalE(Matrix &E, mesh m){
    float l = m.getParameter(ELEMENT_LENGTH);
    float ipsilon = m.getParameter(IPSILON);
    E.at(0).at(0) += ipsilon/l;      E.at(0).at(1) += -ipsilon/l;
    E.at(1).at(0) += -ipsilon/l;     E.at(1).at(1) += ipsilon/l;
}
...
Matrix createLocalK(int element,mesh &m){
    ...
    row1.push_back(C.at(0).at(0)+E.at(0).at(0));
    row1.push_back(C.at(0).at(1)+E.at(0).at(1));

    row2.push_back(A.at(1).at(0)+B.at(1).at(0));
    row2.push_back(A.at(1).at(1)+B.at(1).at(1));
    row2.push_back(C.at(1).at(0)+E.at(1).at(0));
    row2.push_back(C.at(1).at(1)+E.at(1).at(1));
...
Vector createLocalb(int element,mesh &m){
    ...
    float psi = m.getParameter(PSI), l = m.getParameter(ELEMENT_LENGTH), ita = m.getParameter(ITA);
